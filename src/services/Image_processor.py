...
if __name__ == "__main__":
    ...
    if not sample_images:
        ...
        print(f"[TEST] Found {len(sample_images)} sample images")
        sample_path = sample_images[0]
        print(f"[TEST] Processing: {sample_path}")
        features = preprocessor.transform(sample_path)
        display_image = preprocessor.transform_for_display(sample_path)

        print(f"[TEST] Sample image: {sample_path}")
        print(f"[TEST] Flattened feature shape: {features.shape}")
        print(f"[TEST] Display image shape: {display_image.shape}")
        print(f"[TEST] Minimum pixel value: {features.min():.4f}")
        print(f"[TEST] Maximum pixel value: {features.max():.4f}")
